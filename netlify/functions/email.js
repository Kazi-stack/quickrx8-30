...
export const createTransporter = () => {
  // Prefer SMTP_USER / SMTP_PASS (Netlify UI), fallback to GMAIL_USER / GMAIL_APP_PASSWORD
  ...
  if (!user || !pass) {
    throw new Error('SMTP_USER and SMTP_PASS (or GMAIL_USER and GMAIL_APP_PASSWORD) not set. Check your Netlify environment variables.');
  }
...
Correlation ID: ${correlationId}
  ...
  const fromAddr = process.env.SMTP_USER || process.env.GMAIL_USER || process.env.EMAIL_USER;
  const toAddr = process.env.TO_EMAIL || process.env.EMAIL_TO || fromAddr;
